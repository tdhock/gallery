...
myread <- function(csv, repo.col){
  nc::capture_first_df(
    fread(csv),
    viz_user_repo=list(user=".*?", "/", repo_only=".*")
  )[
  , img.png := sprintf("repos/%s.png", viz_user_repo)
  ][
  , viz.link := ahref(
    sprintf("https://%s.github.io/%s/", user, repo_only),
    ifelse(
      file.exists(img.png),
      sprintf('<img src="%s" alt="%s" />', img.png, viz_user_repo),
      viz_user_repo)
  )][
  , repo.link := ahref(
    sprintf("https://github.com/%s/tree/gh-pages", viz_user_repo),
    get(repo.col))
  ]
}
...
error.dt <- myread("error.csv", "viz_user_repo")
```
