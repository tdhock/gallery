  ...
  , img.png := sprintf("repos/%s.png", viz_owner_repo)
    ...
    viz.link = ahref(
      ...
      ifelse(
        ...
        viz_owner_repo)),
    display.title = get(repo.col),
    repo.link = ahref(
      sprintf("https://github.com/%s/tree/gh-pages", viz_owner_repo),
      "repo")
  )][]
}
meta.dt <- myread("meta.csv", "title")[, let(
  source.link = ahref(source, "source"),
  video.link = ifelse(is.na(video), "", ahref(video,  "video"))
)][]
...
```{r}
knitr::kable(meta.dt[, .(
  viz.link, display.title, repo.link, source.link, video.link
)])
...
knitr::kable(error.dt[, .(
  viz.link, display.title, repo.link, error
)])
download("error.csv")
```
